...
//    }
    ...
    public Children addChildTotheparent(@PathVariable Integer parentid, @RequestBody ChildDTO childDTO){
        childDTO.setParent_Id(parentid);
        return parentService.addChildToTheParent(childDTO);
    }

    @GetMapping("/getChildBy/{email}")
    public ResponseEntity  getChildrenByParentEmail(@PathVariable String email ) {
        Children childer = parentService.getChildernsByParentEmail(email);
        return ResponseEntity.status(200).body(childer);
    }
