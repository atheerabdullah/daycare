...
public class BookingService {
    ...
    //Rating
    public void rateService(Integer parentid, Integer bookIdid, RateBookDTO rateBookDTO){
        Parent parent = parentRepository.findParentById(parentid);
        Booking booking=bookingRepository.findBookingById(bookIdid);
        Booking bookingR = bookingRepository.findBookingById(rateBookDTO.getBookId());
        if (parent.getBookingSet() == null || booking.getStatus().equalsIgnoreCase("inProgress")) {
            throw new ApiException("Sorry YOu do NOT have a booking Or Yor Booking InProgress You Can't make a rate");
        }
        booking.setRate(rateBookDTO.getRating());
        bookingRepository.save(booking);
    }
