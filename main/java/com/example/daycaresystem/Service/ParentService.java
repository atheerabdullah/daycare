...
import com.example.daycaresystem.DTO.ChildDTO;
...
//    }
    ...
    public Children addChildToTheParent(ChildDTO childDTO) {
        Parent parent = parentRepository.findParentById(childDTO.getParent_Id());
        if (parent == null) {
            throw new ApiException("Parent Not found");
        }
        Children childern = new Children();
        childern.setChildName(childDTO.getChildName());
        childern.setAge(childDTO.getAge());
        childern.setGender(childDTO.getGender());

        childern.setParent(parent);
        parent.getChildrenSet().add(childern);
        childern = childrenRepository.save(childern);
        return childern;
    }

    public Children getChildernsByParentEmail(String email) {
        Children childern =childrenRepository.findChildernsByParentEmail(email);
        if(childern==null){
            throw new ApiException("parent with The E-mail you entered  Not Found");
        }
        return childrenRepository.findChildernsByParentEmail(email);
    }
